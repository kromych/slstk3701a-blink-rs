#[doc = "Register `ETMPIDR0` reader"]
pub type R = crate::R<Etmpidr0Spec>;
#[doc = "Field `PARTNUM` reader - Part Number"]
pub type PartnumR = crate::FieldReader;
impl R {
    #[doc = "Bits 0:7 - Part Number"]
    #[inline(always)]
    pub fn partnum(&self) -> PartnumR {
        PartnumR::new((self.bits & 0xff) as u8)
    }
}
#[doc = "Peripheral ID0 Register\n\nYou can [`read`](crate::Reg::read) this register and get [`etmpidr0::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct Etmpidr0Spec;
impl crate::RegisterSpec for Etmpidr0Spec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`etmpidr0::R`](R) reader structure"]
impl crate::Readable for Etmpidr0Spec {}
#[doc = "`reset()` method sets ETMPIDR0 to value 0x25"]
impl crate::Resettable for Etmpidr0Spec {
    const RESET_VALUE: u32 = 0x25;
}
