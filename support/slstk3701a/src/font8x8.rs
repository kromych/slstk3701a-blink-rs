//! 8x8 bitmap font for printable ASCII characters (32–126).
//!
//! Each character is stored as 8 bytes (one per row, top to bottom).
//! Bit 7 (MSB) is the leftmost pixel. The display driver reverses bits to
//! match the Sharp Memory LCD's LSB-first pixel ordering.

/// Font data for ASCII 32 (' ') through 126 ('~'). Index with `(ch - 32)`.
#[rustfmt::skip]
...
/// Reverse bits in a byte (MSB-left → LSB-left conversion).
#[inline]
